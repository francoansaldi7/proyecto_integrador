...
const UpdateFormCharacteristics = ({ closeForm, characteristic }) => {

  const [iconName, setIconName] = useState(characteristic.iconName);
  const [characteristicName, setCharacteristicName] = useState(characteristic.name);
  const [errors, setErrors] = useState([]);
  const [isLoading, setIsLoading] = useState(false);

  const icon = useRef(null);

  const { updateCharacteristic } = useContext(GlobalContext);
  ...
  const handleSubmit = async (event) => {
    ...
    if(!(icon?.current && icon.current.childNodes.length > 0)){
      setErrors((prev) => [...prev, "Debes seleccionar un icono"]);
      return;
    }
    setIsLoading(true);
    try {
      savedCharacteristic = await updateCharacteristic(characteristic.id ,{
      ...
      setIsLoading(false);
      toast.error("Se produjo un error al actualizar la característica.");
    }
...
UpdateFormCharacteristics.propTypes = {
  closeForm: PropTypes.func.isRequired,
  characteristic: PropTypes.object.isRequired,
};

export default UpdateFormCharacteristics;
