...
import CreateFormCharacteristics from './CreateFormCharacteristics';
import UpdateFormCharacteristics from './UpdateFormCharacteristics';
const ListOfCharacteristics = () => {
  const {characteristics, setCharacteristics, getAllCharacteristics, saveCharacteristic, updateCharacteristic, deleteCharacteristic} = useContext(GlobalContext)
    ...
    const handleDelete = async () => {
      ...
      try {
        const result = await deleteCharacteristic(idToDeleteParsed);
        if (result) {
          setActiveDeleteForm(!activeDeleteForm);
          setCharacteristics(characteristics.filter((category) => category.id !== idToDelete));
          toast.success("¡Característica eliminada exitosamente!");
          return;
        } 
        toast.error("Se produjo un error al eliminar la característica.");
      } catch (error) {
        console.log(error);
        toast.error("Se produjo un error al eliminar la característica.");
      }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden max-w-[95vw]">
      <ToastContainer
        ...
        theme="dark"
      />
      {activeUpdateForm && <UpdateFormCharacteristics closeForm={closeUpdateForm} characteristic={ characteristics.find((characteristic) => characteristic.id === idToUpdate) } />}
      {activeDeleteForm && <DeleteModal handleDeleteModal={closeDeleteForm} handleDelete={handleDelete} />}
      {activeCreateForm && <CreateFormCharacteristics closeForm={closeCreateForm} />}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
        <div className="flex-1 flex items-center space-x-2">
          <h5>
            <span className="text-gray-500">Todos las características:</span>
                ...
                <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                  <div className="flex items-center space-x-4">
                    <button
                      ...
                      onClick={() => {
                        handleUpdateModal(characteristic.id);
                      }}
                    ...
                    <button
                      ...
                      onClick={() => {handleDeleteModal(characteristic.id);}}
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className="h-4 w-4 mr-2 -ml-0.5"
